...
# Define file paths for input and output. file
#"When you change the input file path, remember there are 3 places have to be changed in this programm #line18 306 325"
input_file_path = '/Users/mac/Desktop/WBG experiment/Python/PL Data analyze/250320/Remove noise csv/Nr 86.csv'
...
for i, (cycle_time, intensity) in enumerate(cycle_data.items()):
    ...
    # Define dynamic fitting range for Peak 2 based on the cycle number
    # Define dynamic fitting range for Peak 2 based on the cycle number
    if i < 2:
        peak2_range = (wavelength >= 605) & (wavelength <= 800)
    elif 2 <= i <= 4:
        peak2_range = (wavelength >= 705) & (wavelength <= 880)
    elif 5 <= i <= 31:
        peak2_range = (wavelength >= 730) & (wavelength <= 880)
    else:  # i >= 50 and i <= 90
        peak2_range = (wavelength >= 730) & (wavelength <= 880)
    ...
    sigma_guess2 = (max(x_peak2) - min(x_peak2)) / 100

    try:
        popt2, _ = curve_fit(
            gaussian, x_peak2, y_peak2,
            p0=[A_guess2, mu_guess2, sigma_guess2],
            bounds=([0, 600, 0.5], [np.inf, 900, 100]),
            maxfev=10000
        )
...
for column in ['Peak 1 Position', 'Peak 1 Intensity', 'Peak 1 Area', 'Peak 1 FWHM',
    ...
    for idx in fitting_df[nan_indices].index:
        ...
        if not pd.isna(prev_idx) and not pd.isna(next_idx):
            ...
            fitting_df.loc[idx, column] = prev_value + (next_value - prev_value) * (idx - prev_idx) / (next_idx - prev_idx)
...
fitting_df['Time'] = [0] + [(datetime.strptime(real_time[i], '%m/%d/%Y %H:%M:%S') - datetime.strptime(real_time[0], '%m/%d/%Y %H:%M:%S')).total_seconds() for i in range(1, len(real_time))]
...
for idx, color, label in zip(selected_indices, selected_colors, labels):
    ...
    if fitting_df.loc[idx, 'Peak 2 Position'] > 0:
        peak2_fit_x = wavelength[(wavelength >= 510) & (wavelength <= 950)]
...
photo_dir = "/Users/mac/Desktop/WBG experiment/Python/PL Data analyze/250320/PVK Photos"
...
recipe_path = "/Users/mac/Desktop/WBG experiment/Python/PL Data analyze/250320/Formatted Recipes.csv"
...
shift_x_nm = round(fitting_df.loc[fitting_df['Cycle Time'] == 83, 'Peak 2 Position'].values[0] - fitting_df.loc[fitting_df['Cycle Time'] == 1, 'Peak 2 Position'].values[0])
